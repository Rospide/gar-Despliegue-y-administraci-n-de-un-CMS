...
if [ -z "$1" ]; then
    echo "Uso: ./crear_frontend1.sh <usuario_vm>"
    echo "Ejemplo: ./crear_frontend1.sh alumno"
    exit 1
fi
...
VBoxManage modifyvm "$VM_NAME" --natpf1 delete "ssh-frontend1" 2>/dev/null || true
VBoxManage modifyvm "$VM_NAME" --natpf1 "ssh-frontend1,tcp,,${SSH_PORT},,22"
...
echo
echo "frontend1 creado y arrancado."
echo
...
echo "scp -P ${SSH_PORT} ../scripts/configurar_frontend.sh ${VM_USER}@127.0.0.1:~/"
